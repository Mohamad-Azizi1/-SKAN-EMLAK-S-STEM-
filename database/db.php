...
function sql(&$db, $tablo, $where = '', $orderby = '', $yildiz = '*', $limit = '') { 
  $sql = "SELECT ".$yildiz." FROM ";
  $sql .= $tablo;
  ...
  if($limit != ''){
    $sql .= " LIMIT ".$limit; // LIMIT parametresi ekleniyor
  }
...
$projeler = sql( 
  $db, 
  'projeler LEFT JOIN proje_kategori ON projeler.kategori_id = proje_kategori.id', 
  'proje_kategori.config = "tamamlanan_projeler"', 
  '', 
  'projeler.*', // Kolon seçimi
  '1' // LIMIT 1
);
